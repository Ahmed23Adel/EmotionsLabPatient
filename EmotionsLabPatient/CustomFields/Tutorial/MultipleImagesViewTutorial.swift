...
struct SingleImageWrapperTutorial: View {
    @ObservedObject var image: SingleImage
    var selectCurrentImageParentFunc: (SingleImage) -> Void
    
    init(image: SingleImage, selectCurrentImageParentFunc: @escaping (SingleImage) -> Void) {
        self.image = image
    ...
    var body: some View {
        Spacer()
        SingleImageTutorialWithAnimationView(currentImage: image, selectCurrentImageParentFunc: selectCurrentImageParentFunc)
        Spacer()
    }
}

struct ImageRowTutorial: View {
    let rowImages: [SingleImage]
    var selectCurrentImageParentFunc: (SingleImage) -> Void
    var body: some View {
        HStack {
            ForEach(Array(rowImages.enumerated()), id: \.offset) { pair in
                SingleImageWrapperTutorial(image: pair.element, selectCurrentImageParentFunc: selectCurrentImageParentFunc)
            }
        }
    }
}

struct MultipleImagesViewTutorial: View {
    ...
    var body: some View {
        VStack {
            ...
            ForEach(Array(rows.enumerated()), id: \.offset) { pair in
                ImageRowTutorial(rowImages: pair.element, selectCurrentImageParentFunc: selectCurrentImageParentFunc)
            }
        }
    }
